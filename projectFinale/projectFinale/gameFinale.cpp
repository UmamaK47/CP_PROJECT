...
void warrior_quest2() {
	...
	cout << "do you want to exit quest?\n press 1 if yes \n";
	cin >> choice_exit;
	if (choice_exit == 1) {
		exit_game();
	}
...
void warrior_quest3() {
	...
	character.xp += 25;
	character.level++;
	cout << "\nLevel Upgraged! Your new level is: " << character.level;
	cout << "\ndo you want to exit?\n press 1 if yes \n";
	cin >> choice_exit;
	if (choice_exit == 1) {
		exit_game();
...
void rogueQuest2() {
	...
	if (enemy_health.enemyHealth > 0) {
		...
		do {
			if (user_input1 == 1) {
				rogue_inventory + "\t key";
				cout << "\nkey to throne room\;s key is added to inventory\n";
			}
			else {
				cout << "\ninvalid command";
			}
...
void rogueQuest3() {
	...
	cout << "You are on the back of the castle to jump from the windows and go to the secret room un noticed.\n";
	while (character.health > 0 && enemy_health.enemyHealth > 0)
	{
		cout << "You’ve reached 4th floor you are noticing the guards now you have 2 options:\n1	attack from your dagger\n2	attack from arrow	\n3	evade\n";
		cout << "\n------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n";
		int choice;
		cin >> choice;
